...
func (receiver *DimPayChannelModel) Take(ctx context.Context, fields string, query string, args ...interface{}) (err error) {
	cacheKey := cryptor.Md5String(cache.BuildCacheKey("pay-channel-take-", fields, query, args))
	...
	takeErr := receiver.DimPayChannelModel.DimPayChannelModel.Take(ctx, fields, query, args...)
	if takeErr != nil {
		err = takeErr
